...
def update_progress(progress, block):
    ...
    text = "\rPercent:  [{0}] {1}% | Number:  {2} {3}".\
        format( "#"*block + "-"*(bar_length-block), np.round((progress*100),1), block, status)
...
def load_file(filename):
    ...
    try:
        ...
        if not all(key in simulation_config_parameters.keys() for key in ['no_of_blocks','lambda','no_of_agents',\
                                                                      'alpha','latency','distance','tip_selection_algo',\
                                                                      'agent_choice','printing']):

            print("Parameter error! Please provide 'no_of_blocks','lambda','no_of_agents','alpha','latency',"
            "'distance','tip_selection_algo','agent_choice','printing'!")
            sys.exit(1)

        #Load simulation parameters
        _no_of_blocks = int(simulation_config_parameters['no_of_blocks'])
        ...
        data.append((_no_of_blocks, _lambda, _no_of_agents, \
...
def csv_export(self):
    ...
    with open('data.csv', 'w', newline='') as file:
        ...
        for block in self.DG.nodes:
            #Write all other block
            if(block.arrival_time != 0):
                line = []
                line.append(block)
                line.append(list(self.DG.successors(block)))
                # line.append(block.arrival_time)
                # line.append(block.agent)
